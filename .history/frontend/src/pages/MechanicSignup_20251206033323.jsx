...
import { sendOtp, registerMechanic } from "../api/mechanicApi"; // loginMechanic not needed here
...
export default function MechanicSignup() {
  ...
  // ✅ Phone validation removed (optional now)
  const validateStep1 = () => {
 ...
 const handleVerifyAndRegister = async () => {
  ...
  try {
    ...
    // Build payload with optional phone
    const payload = {
      name: form.name.trim(),
    ...
    // Agar phone invalid ya empty ho, payload me add mat karo

    await registerMechanic(payload);

    // Email OTP verification (if your flow needs it post-register)
    // Tip: Agar backend register hi OTP verify karta hai, ye call redundant ho sakti hai.
    // await axios.post("/api/otp/send-email-otp", { email: form.email.trim(), role: "mechanic" });

    // Get token from register response (adjust as per your API)
              ...
              <div className="space-y-4">
                ...
                <button
                  onClick={async () => {
                    try {
                      setLoading(true);
                      const res = await sendOtp({ email: form.email }); // ✅ resend via email
                      setServerOtp(res.data?.otp || null);
                      setLoading(false);
