...
export default function MechanicSignup() {
  ...
  const handleSendOtp = async () => {
    ...
    try {
      setLoading(true);
      // Use email OTP (preferred). API supports phone or email.
      const res = await sendOtp({ email: form.email.trim() });

      // backend returns debugOtp when DEBUG_SEND_OTP=true
      const debug = res?.data?.debugOtp || res?.data?.otp || null;
      if (debug) setServerOtp(String(debug));
  ...
  const handleVerifyAndRegister = async () => {
    ...
    // If DEBUG and serverOtp available, optionally auto-validate client-side while testing
    // but we'll still send to backend for real verification
    try {
      setLoading(true);

      // build payload; phone is optional (only include if valid 10 digits)
      const payload = {
        name: form.name.trim(),
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 flex justify-center items-center px-4 py-10">
      <div className="max-w-4xl w-full bg-white dark:bg-gray-800 shadow-2xl rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-700">
        <div className="grid grid-cols-1 md:grid-cols-2">
          <div className="hidden md:flex flex-col justify-center bg-gradient-to-br from-indigo-600 to-blue-500 text-white p-10 rounded-l-2xl">
            ...
            <p className="text-sm opacity-90 mb-8">Get more jobs, manage customers professionally and grow your garage.</p>
          ...
          <div className="p-8 md:p-10">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">Create your mechanic account to start receiving service requests.</p>
            ...
            {step === 1 && (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <input placeholder="Phone Number (optional)" className="input" value={form.phone} onChange={(e) => handle("phone", e.target.value)} />
                  <input placeholder="GST Number (optional)" className="input" value={form.gst} onChange={(e) => handle("gst", e.target.value)} />
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <input placeholder="Password" type="password" className="input" value={form.password} onChange={(e) => handle("password", e.target.value)} />
                  <input placeholder="Confirm Password" type="password" className="input" value={form.confirmPassword} onChange={(e) => handle("confirmPassword", e.target.value)} />
                </div>

                <textarea placeholder="Complete Garage Address" className="input h-24 resize-none" value={form.address} onChange={(e) => handle("address", e.target.value)} />

                <div className="flex items-center gap-4 mt-4">
                  <button onClick={handleSendOtp} disabled={loading} className="btn w-full">
            ...
            {step === 2 && (
              <div className="space-y-4">
                <p className="text-gray-600 dark:text-gray-400 text-sm">OTP sent to <strong>{form.email}</strong></p>

                <div className="flex gap-3">
                  <input placeholder="Enter OTP" className="input" value={form.otp} onChange={(e) => handle("otp", e.target.value)} />
                  <button onClick={() => setStep(1)} className="px-4 py-2 border rounded-lg dark:border-gray-600">Edit</button>
                </div>
                ...
                <button onClick={handleSendOtp} className="w-full text-sm mt-2 text-indigo-500">Resend OTP</button>

                {serverOtp && (
                  <div className="text-xs text-gray-500 mt-2">DEBUG OTP (dev only): {serverOtp}</div>
                )}
              </div>
            )}
          </div>
      ...
      <style>{`
        .input { width: 100%; padding: 12px 14px; border-radius: 10px; border: 1px solid #d1d5db; background: transparent; color: inherit; }
        .input:focus { border-color: #6366f1; outline: none; }
        .btn { background: linear-gradient(90deg,#6366f1,#06b6d4); padding: 12px 14px; border-radius: 10px; font-weight: 600; color: white; }
      `}</style>
    </div>
  );
}
