...
const Navbar = () => {
  ...
  // Auth
  const token = localStorage.getItem("token"); // FIX: correct token key
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-6">
            ...
            {!token && (
              <>
                {/* Become Mechanic (only when NOT logged in) */}
                <Link
            ...
            {token && (
              <div className="relative">
                ...
                {dropdown && (
                  <div
                    className="absolute right-0 mt-3 bg-white dark:bg-gray-800 shadow-xl rounded-xl w-44 overflow-hidden border border-gray-200 dark:border-gray-700"
                  >
                    {/* Dashboard Link */}
                    <button
                      onClick={() => {
                    ...
                    >
                      {role === "mechanic" ? <FaTools /> : <FaUserCircle />}
                      Dashboard
                    </button>
      ...
      {isOpen && (
        <div className="md:hidden bg-gray-800 px-4 py-4 space-y-3 shadow-lg">
          ...
          <Link to="/shop" className="mobile-link flex items-center gap-2" onClick={() => setIsOpen(false)}>
            <FaShoppingBag /> Shop
          </Link>
