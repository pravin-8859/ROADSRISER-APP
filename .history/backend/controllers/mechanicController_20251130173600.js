...
export const sendOtp = async (req, res) => {
  try {
    ...
    // Save without full validation because many fields are optional at OTP stage
    await mech.save({ validateBeforeSave: false });

    console.log("MECHANIC DEBUG OTP:", otp);
    // Return debugOtp for development (remove in production)
...
/** SIGNUP — uses the existing OTP doc (update, do NOT create a new doc) */
export const mechanicSignup = async (req, res) => {
  try {
    ...
    // If email provided — ensure not used by some other mechanic
    if (email) {
    ...
    if (err?.code === 11000) {
      return res.status(400).json({ message: "Duplicate field", error: err.keyValue });
    }
...
/** refresh, logout, resetPassword (keep as you had) */
export const refreshAccessToken = async (req, res) => {
  try {
    ...
    if (!mech || mech.refreshToken !== token) return res.status(401).json({ message: "Invalid refresh token" });

    const newToken = createAccessToken(mech._id);
