...
export const sendOtp = async (req, res) => {
  try {
    ...
    const otp = generateOtp(4); // or fixed => "1234"
    ...
    mech.otpExpire = new Date(expire);
    mech.otpPurpose = purpose;
    await mech.save();
    ...
    return res.json({
      ...
      otp, // visible for development
    });
  } catch (err) {
    console.error(err);
    return res.status(500).json({
      message: "Failed (debug mode) — internal error",
    });
  }
};
